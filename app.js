(() => {
  ...
  function render(items){reviewsEl.innerHTML='';emptyEl.hidden=items.length>0;summaryEl.hidden=items.length===0;if(!items.length)return;const avg=items.reduce((s,r)=>s+Number(r.rating||0),0)/items.length;averageEl.textContent=avg.toFixed(1);starsEl.textContent='★'.repeat(Math.round(avg))+'☆'.repeat(5-Math.round(avg));countEl.textContent=`${items.length} ${items.length===1?'review':'reviews'}`;items.forEach(r=>{const card=document.createElement('article');card.className='review-card';const stars='★'.repeat(Number(r.rating))+'☆'.repeat(5-Number(r.rating));card.innerHTML=`<div class="review-stars" aria-label="${Number(r.rating)} out of 5 stars">${stars}</div><h3>${escapeHTML(r.title)}</h3><p class="review-text">“${escapeHTML(r.review)}”</p>${r.favourite?`<p class="review-favourite"><strong>Favourite bit:</strong> ${escapeHTML(r.favourite)}</p>`:''}<div class="review-meta"><span><strong>${escapeHTML(r.name)}</strong><small>${formatMonth(r.stay_date)}</small></span><small>${r.return_answer?`<strong>Would you stay again?</strong><br>${escapeHTML(r.return_answer)}`:''}</small></div>`;reviewsEl.appendChild(card)});}
